...
id="$1"; url="$2"; out="masters/v5.15/$id.mp3"
curl -sS -o "$out" "$url" || { echo "FAIL $id"; exit 1; }
d=$(ffprobe -v error -show_entries format=duration -of csv=p=0 "$out")
node -e "
const fs=require('fs');const p=JSON.parse(fs.readFileSync('masters/v5.15/progress.json'));
const l=p.lines.find(l=>l.id==='$id');l.status='done';l.secsNew=+(+'$d').toFixed(2);
fs.writeFileSync('masters/v5.15/progress.json',JSON.stringify(p,null,1));"
