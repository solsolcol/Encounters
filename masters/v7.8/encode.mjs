// encode.mjs [--install] [ids...] — v7.8: the 26 teen lines (Aaron) to the contract, from
// masters/v7.8/pick.json. Voice: mono 44.1k/128k mp3 + 48k/64k opus, peak-matched to chapter
...
const M = 'masters/v7.8', OUT = '/tmp/claude-0/-home-user-Encounters/f1062235-ca86-59a9-b000-61422c281293/scratchpad/enc78';
...
const report = [];
for (const p of plan) {
  const oldMp3 = `assets/audio/${p.id}.mp3`;
  if (!fs.existsSync(oldMp3) || !fs.existsSync(`assets/audio-opus/${p.id}.ogg`)) throw new Error('no shipped file to replace for ' + p.id);
  const pk = peak(p.src);
  ...
  const row = { id: p.id, src: p.src, srcPeak: pk, gain: +gain.toFixed(2), peak: peak(mp3), oldSecs: +dur(oldMp3).toFixed(2), secs: +dur(mp3).toFixed(2), kb: (fs.statSync(mp3).size / 1024 | 0) + '/' + (fs.statSync(ogg).size / 1024 | 0) };
  report.push(row);
