...
# large result to disk) and download each take as masters/v7.8/<id>-<a|b>.mp3,
# mapping session -> id through sessions.json. Idempotent: skips files present.
import json, glob, re, subprocess, os, sys
S = json.load(open('masters/v7.8/sessions.json'))
sess2id = {}
for group in ('lines', 'sfx', 'music'):
    for id_, sids in S.get(group, {}).items():
        for k, sid in enumerate(sids): sess2id[sid] = (id_, 'ab'[k])
files = [f for f in glob.glob('/root/.claude/projects/-home-user-Encounters/f1062235-ca86-59a9-b000-61422c281293/tool-results/mcp-ElevenLabs-creative_get_flow_run_status-*.txt') if int(re.search(r'-(\d+)\.txt$', f).group(1)) >= 1789045186503]
seen, pending, failed = {}, [], []
for f in files:
    ...
    for g in st.get('generations', []):
        if g.get('status') not in ('completed',): pending.append((g.get('status'), g.get('prompt','')[:40]))
...
for sid, (id_, take) in sess2id.items():
    dst = f'masters/v7.8/{id_}-{take}.mp3'
