/* Master Zav, v5.09: same pipeline as v5.08 (prepzav.mjs) plus UV-island
...
import { dedup, prune, weld, simplify, textureCompress, quantize, meshopt } from '@gltf-transform/functions';
...
const BAND = Number(process.env.BAND || 4), DEEP = Number(process.env.DEEP || 6), THRESH = Number(process.env.THRESH || 8);
{
  ...
  for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) {
    ...
    rim++;
    // the island's interior nearby: mean colour of pixels at least DEEP in
    let sum = 0, k = 0, r = 0, g = 0, b = 0;
    ...
    /* v5.10: a halo pixel is REPAINTED with the island's own interior colour,
       ...
       streak along the seam that the original never had (Chad saw it) */
    if (lum(i) > sum / k + THRESH) { px[i * C] = r / k; px[i * C + 1] = g / k; px[i * C + 2] = b / k; stripped++; }
  }
...
// --- 3. geometry: keep RATIO of the triangles (1 = all of them), quantize,
//        and meshopt-compress (v5.10: Chad's call — the full-resolution man)
...
const ratio = Number(process.argv[2] || 1);
const steps = [dedup(), weld()];
if (ratio < 1) steps.push(simplify({ simplifier: MeshoptSimplifier, ratio, error: 0.01 }));
steps.push(textureCompress({ encoder: sharp, targetFormat: 'jpeg', quality: 86, resize: [1024, 1024] }), prune());
await doc.transform(...steps);
const outName = process.argv[4] || 'zav.full.glb';
await doc.transform(quantize());
await io.write(outName.replace('.glb', '.quant.glb'), doc);
await MeshoptEncoder.ready;
await doc.transform(meshopt({ encoder: MeshoptEncoder, level: 'medium' }));
await io.write(outName, doc);
const fs = await import('node:fs');
console.log('ratio', ratio, 'tris:', root.listMeshes().map(m => m.listPrimitives().map(p => p.getIndices().getCount() / 3).join(',')).join(' '),
  'meshopt KB', fs.statSync(outName).size / 1024 | 0, 'quantized-only KB', fs.statSync(outName.replace('.glb', '.quant.glb')).size / 1024 | 0,
